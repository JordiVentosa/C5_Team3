...
EVAL_RUNNERS = {
    "point_prompt":        run_point_prompt_inf,
    "bbox_grounded":       run_text_prompt_inf,
    "bbox_detection" :     run_bbox_best_inf
...
def main():
    ...
    inference_type  = inference_cfg["type"]
    output_dir = Path(inference_cfg["output_dir"])
    output_dir.mkdir(parents=True, exist_ok=True)
    
    print(f"\nRunning inference: {inference_type}, output saved at {output_dir}")
 
    if inference_type not in EVAL_RUNNERS:
        raise ValueError(f"Unknown evaluation type '{inference_type}'. "
                         f"Available: {list(EVAL_RUNNERS)}")
 
    # pass any extra config keys as kwargs to the runner
    extra = {k: v for k, v in inference_cfg.items() if k not in ("type", "output_dir")}
    EVAL_RUNNERS[inference_type](coco_gt, coco_dict, model, output_dir, **extra)
    print("\nDone.")
