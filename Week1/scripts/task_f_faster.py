...
def main():
    ...
    data = load_dataset("biglam/european_art", split="train", trust_remote_code=True)
    data = data.train_test_split(test_size=0.2, seed=42)

    dataset = EuropeanArtDataset(  transform=train_transform,dataset=data["train"])
    val_dataset = EuropeanArtDataset(dataset=data["test"])
